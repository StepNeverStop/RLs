...
class DDPG(Policy):
    def __init__(self,
        ...
        with self.graph.as_default():
            ...
            self.q_actor, _ = self._build_q_net(
                'q', self.s_mu, True, reuse=True)
            ...
            with tf.control_dependencies([self.train_actor]):
                self.assign_q_target = tf.group([tf.assign(
                    r, self.ployak * v + (1 - self.ployak) * r) for r, v in zip(self.q_target_var, self.q_var)])
            ...
            #     tf.assign(r, 1/(self.global_step+1) * v + (1-1/(self.global_step+1)) * r) for r, v in zip(self.q_target_var, self.q_var)]
            # self.assign_q_target = [
