        ...
        with tf.device(self.device):
            ...
            with tf.GradientTape() as tape:
                if self.action_type == 'continuous':
                    ...
                    a_new = tf.nn.softmax((logp_all + gumbel_noise) / 0.1)
                    a_s_log_prob = tf.reduce_sum(tf.multiply(logp_all, a_new), axis=1, keepdims=True)
        ...
        with tf.device(self.device):
            with tf.GradientTape(persistent=True) as tape:
                if self.action_type == 'continuous':
                    ...
                    gumbel_noise = tf.cast(self.gumbel_dist.sample([a.shape[0], self.a_counts]), dtype=tf.float64)
                    a_new = tf.nn.softmax((logp_all + gumbel_noise) / 0.1)
                    a_s_log_prob = tf.reduce_sum(tf.multiply(log_action_probs, a_new), axis=1, keepdims=True)
                    entropy = tf.reduce_mean(tf.reduce_sum(tf.exp(logp_all) * logp_all, axis=1, keepdims=True))
