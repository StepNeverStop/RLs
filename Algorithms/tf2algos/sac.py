...
class SAC(Policy):
    def __init__(self,
        ...
        self.v_net = Nn.critic_v('v')
        self.v_target_net = Nn.critic_v('v_target')
        tf.group([r.assign(self.ployak * v + (1 - self.ployak) * r) for r, v in zip(self.v_target_net.weights, self.v_net.weights)])
