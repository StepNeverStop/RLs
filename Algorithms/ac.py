...
　　　ｘｘｘ　　ｘｘｘｘｘ　　　　　　　ｘｘｘｘｘｘ　　　　　　　　
    ...
    def store_data(self, s, a, r, s_, done):
        ...
        self.data.add(s, a, old_prob, r[:, np.newaxis], s_, done[:, np.newaxis])
        # self.off_store(s, a, old_prob, r[:, np.newaxis], s_, done[:, np.newaxis])
    ...
    def no_op_store(self, s, a, r, s_, done):
        ...
        if self.policy_mode == 'OFF':
            self.data.add(s, a, old_prob[:, np.newaxis], r[:, np.newaxis], s_, done[:, np.newaxis])

    def learn(self, episode):
        s, a, old_prob, r, s_, done = self.data.sample()
        pl_visual_s, pl_s = self.get_visual_and_vector_input(s)
