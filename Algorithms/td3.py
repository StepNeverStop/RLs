    ...
    def __init__(self,
        ...
        with self.graph.as_default():
            ...
            self.mu, self.action, self.actor_var = self._build_actor_net(
                'actor', self.pl_s, trainable=True)
            ...
            self.q1_actor, _ = self._build_q_net(
                'q1', self.s_mu, True, reuse=True)
            ...
            self.dc_r = tf.stop_gradient(
                self.r + self.gamma * self.q_target)
            ...
            with tf.control_dependencies([self.train_value]):
                self.train_actor = optimizer.minimize(
                    self.actor_loss, var_list=actor_vars, global_step=self.global_step)
            with tf.control_dependencies([self.train_actor]):
                self.assign_q1_target = tf.group([tf.assign(
                    r, self.ployak * v + (1 - self.ployak) * r) for r, v in zip(self.q1_target_var, self.q1_var)])
                self.assign_q2_target = tf.group([tf.assign(
                    r, self.ployak * v + (1 - self.ployak) * r) for r, v in zip(self.q2_target_var, self.q2_var)])
                self.assign_actor_target = tf.group([tf.assign(
                    r, self.ployak * v + (1 - self.ployak) * r) for r, v in zip(self.actor_target_var, self.actor_var)])
            # self.assign_q1_target = [
            #     tf.assign(r, 1/(self.global_step+1) * v + (1-1/(self.global_step+1)) * r) for r, v in zip(self.q1_target_var, self.q1_var)]
            # self.assign_q2_target = [
            #     tf.assign(r, 1/(self.global_step+1) * v + (1-1/(self.global_step+1)) * r) for r, v in zip(self.q2_target_var, self.q2_var)]
            # self.assign_actor_target = [
            #     tf.assign(r, 1/(self.global_step+1) * v + (1-1/(self.global_step+1)) * r) for r, v in zip(self.actor_target_var, self.actor_var)]
