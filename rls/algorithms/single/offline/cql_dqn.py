...
class CQL_DQN(DQN):
    ...
    def __init__(self,
                 cql_weight=0.5,
                 **kwargs):
        raise NotImplementedError("Dataset and replay buffer is under implementation. CQL is not for offline training for now.")
        # if you want to test CQL, you could remove the exception above and replace 'offline' to 'off-policy' and test it in online off-policy manner.
    ...
    def _train(self, BATCH):
        ...
        cql1_loss = (t.logsumexp(q, dim=-1, keepdim=True) - q).mean()  # 1
