...
class BCQ(SarlOffPolicy):
    ...
    def select_action(self, obs):
        if self.is_continuous:
            ...
            _actions = t.stack(_actions, dim=0)  # [N, B, A]
            q1s = []
            for i in range(self._select_samples):
                q1s.append(self.critic(obs, _actions[i])[0])
            q1s = t.stack(q1s, dim=0)  # [N, B, 1]
            max_idxs = q1s.argmax(dim=0, keepdim=True)[-1]  # [1, B, 1]
            actions = _actions[max_idxs, t.arange(self.n_copys).reshape(self.n_copys, 1), t.arange(self.a_dim)]
        else:
    ...
    def _train(self, BATCH):
        if self.is_continuous:
            ...
            KL_loss = -0.5 * (1 + t.log(std.pow(2)) - mean.pow(2) - std.pow(2)).mean()
            ...
            for _ in range(self._train_samples):
                ...
                target_Q = self._lmbda * t.min(target_Q1, target_Q2) + (1. - self._lmbda) * t.max(target_Q1, target_Q2)
                target_Qs.append(target_Q)
            target_Qs = t.stack(target_Qs, dim=0)  # [N, T, B, 1]
