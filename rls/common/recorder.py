...
class SimpleMovingAverageRecoder(Recoder):
    ...
    def episode_reset(self):
        ...
        self.total_returns = defaultdict(lambda: np.zeros((self.n_copys, ), dtype=float))
        self.discounted_returns = defaultdict(lambda: np.zeros((self.n_copys, ), dtype=float))
        self.already_dones = defaultdict(lambda: np.zeros((self.n_copys, ), dtype=bool))
        ...
        for id in self.agent_ids:
            ...
            if self.verbose:
                _dicts[id].update(dict([
                    [f'{title}/first_done_step', self.steps[id][self.already_dones[id] > 0].min() if self.has_done else -1],
                    [f'{title}/last_done_step', self.steps[id][self.already_dones[id] > 0].max() if self.has_done else -1]
                ]))
        return _dicts
